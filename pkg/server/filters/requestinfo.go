...
func WithRequestInfo(handler http.Handler, resolver request.RequestInfoResolver) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		...
		if len(authorization) == 0 {
			xAuthorization := req.Header.Get("X-Captain-Authorization")
			if len(xAuthorization) != 0 {
				req.Header.Set("Authorization", xAuthorization)
				req.Header.Del("X-Captain-Authorization")
			}
		}
		...
		if rawQuery := req.Header.Get("X-Captain-Rawquery"); len(rawQuery) != 0 && len(req.URL.RawQuery) == 0 {
			req.URL.RawQuery = rawQuery
			req.Header.Del("X-Captain-Rawquery")
		}
